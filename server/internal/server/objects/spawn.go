...
func isTooClose[T any](x float64, y float64, radius float64, object *SharedCollection[T], getPosition func(T) (float64, float64), getRadius func(T) float64) bool {
	if object == nil {
		return false
	}
	tooClose := false
	object.ForEach(func(_ uint64, object T) {
		if tooClose {
			return
		}
